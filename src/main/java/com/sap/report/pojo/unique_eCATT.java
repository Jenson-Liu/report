package com.sap.report.pojo;

/**
 * @author : Jenson.Liu
 * @date : 2019-07-28  09:18
 */

public class unique_eCATT {

    String accid;
    String text;
    String ecattcallingbcset;
    String containsdeliveryclassa;
    String intablemdascustd;
    String usedonlyindemodata;
    String usedinoponly;
    String lobresponsibleforecatt;
    String abapreplacementclass;
    String applicationcomponentofrepleacementclass;
    String lobresponsibleforreplacementclass;
    String ecattclassificationdonebylob;
    String responsiblelob;
    String lobcomments;
    String loccommitmentstatuspubliccloud1911;

    public unique_eCATT(String accid, String text, String ecattcallingbcset, String containsdeliveryclassa, String intablemdascustd, String usedonlyindemodata, String usedinoponly, String lobresponsibleforecatt, String abapreplacementclass, String applicationcomponentofrepleacementclass, String lobresponsibleforreplacementclass, String ecattclassificationdonebylob, String responsiblelob, String lobcomments, String loccommitmentstatuspubliccloud1911) {
        this.accid = accid;
        this.text = text;
        this.ecattcallingbcset = ecattcallingbcset;
        this.containsdeliveryclassa = containsdeliveryclassa;
        this.intablemdascustd = intablemdascustd;
        this.usedonlyindemodata = usedonlyindemodata;
        this.usedinoponly = usedinoponly;
        this.lobresponsibleforecatt = lobresponsibleforecatt;
        this.abapreplacementclass = abapreplacementclass;
        this.applicationcomponentofrepleacementclass = applicationcomponentofrepleacementclass;
        this.lobresponsibleforreplacementclass = lobresponsibleforreplacementclass;
        this.ecattclassificationdonebylob = ecattclassificationdonebylob;
        this.responsiblelob = responsiblelob;
        this.lobcomments = lobcomments;
        this.loccommitmentstatuspubliccloud1911 = loccommitmentstatuspubliccloud1911;
    }

    public unique_eCATT() {

    }

    public String getAccid() {
        return accid;
    }

    public void setAccid(String accid) {
        this.accid = accid;
    }

    public String getText() {
        return text;
    }

    public void setText(String text) {
        this.text = text;
    }

    public String getEcattcallingbcset() {
        return ecattcallingbcset;
    }

    public void setEcattcallingbcset(String ecattcallingbcset) {
        this.ecattcallingbcset = ecattcallingbcset;
    }

    public String getContainsdeliveryclassa() {
        return containsdeliveryclassa;
    }

    public void setContainsdeliveryclassa(String containsdeliveryclassa) {
        this.containsdeliveryclassa = containsdeliveryclassa;
    }

    public String getIntablemdascustd() {
        return intablemdascustd;
    }

    public void setIntablemdascustd(String intablemdascustd) {
        this.intablemdascustd = intablemdascustd;
    }

    public String getUsedonlyindemodata() {
        return usedonlyindemodata;
    }

    public void setUsedonlyindemodata(String usedonlyindemodata) {
        this.usedonlyindemodata = usedonlyindemodata;
    }

    public String getUsedinoponly() {
        return usedinoponly;
    }

    public void setUsedinoponly(String usedinoponly) {
        this.usedinoponly = usedinoponly;
    }

    public String getLobresponsibleforecatt() {
        return lobresponsibleforecatt;
    }

    public void setLobresponsibleforecatt(String lobresponsibleforecatt) {
        this.lobresponsibleforecatt = lobresponsibleforecatt;
    }

    public String getAbapreplacementclass() {
        return abapreplacementclass;
    }

    public void setAbapreplacementclass(String abapreplacementclass) {
        this.abapreplacementclass = abapreplacementclass;
    }

    public String getApplicationcomponentofrepleacementclass() {
        return applicationcomponentofrepleacementclass;
    }

    public void setApplicationcomponentofrepleacementclass(String applicationcomponentofrepleacementclass) {
        this.applicationcomponentofrepleacementclass = applicationcomponentofrepleacementclass;
    }

    public String getLobresponsibleforreplacementclass() {
        return lobresponsibleforreplacementclass;
    }

    public void setLobresponsibleforreplacementclass(String lobresponsibleforreplacementclass) {
        this.lobresponsibleforreplacementclass = lobresponsibleforreplacementclass;
    }

    public String getEcattclassificationdonebylob() {
        return ecattclassificationdonebylob;
    }

    public void setEcattclassificationdonebylob(String ecattclassificationdonebylob) {
        this.ecattclassificationdonebylob = ecattclassificationdonebylob;
    }

    public String getResponsiblelob() {
        return responsiblelob;
    }

    public void setResponsiblelob(String responsiblelob) {
        this.responsiblelob = responsiblelob;
    }

    public String getLobcomments() {
        return lobcomments;
    }

    public void setLobcomments(String lobcomments) {
        this.lobcomments = lobcomments;
    }

    public String getLoccommitmentstatuspubliccloud1911() {
        return loccommitmentstatuspubliccloud1911;
    }

    public void setLoccommitmentstatuspubliccloud1911(String loccommitmentstatuspubliccloud1911) {
        this.loccommitmentstatuspubliccloud1911 = loccommitmentstatuspubliccloud1911;
    }


    @Override
    public String toString() {
        return "unique_eCATT{" +
                "accid='" + accid + '\'' +
                ", text='" + text + '\'' +
                ", ecattcallingbcset='" + ecattcallingbcset + '\'' +
                ", containsdeliveryclassa='" + containsdeliveryclassa + '\'' +
                ", intablemdascustd='" + intablemdascustd + '\'' +
                ", usedonlyindemodata=" + usedonlyindemodata +
                ", usedinoponly='" + usedinoponly + '\'' +
                ", lobresponsibleforecatt='" + lobresponsibleforecatt + '\'' +
                ", abapreplacementclass='" + abapreplacementclass + '\'' +
                ", applicationcomponentofrepleacementclass='" + applicationcomponentofrepleacementclass + '\'' +
                ", lobresponsibleforreplacementclass='" + lobresponsibleforreplacementclass + '\'' +
                ", ecattclassificationdonebylob='" + ecattclassificationdonebylob + '\'' +
                ", responsiblelob='" + responsiblelob + '\'' +
                ", lobcomments='" + lobcomments + '\'' +
                ", loccommitmentstatuspubliccloud1911='" + loccommitmentstatuspubliccloud1911 + '\'' +
                '}';
    }
}
